#!/usr/bin/env python3
"""Metrics helpers for joint enhancement-detection training."""
...
@dataclass
class RunningJointMetrics:
    """Accumulate losses and binary classification metrics across batches."""

    positive_label: int = 1
    num_classes: int = 2
    total_loss_sum: float = 0.0
    detection_loss_sum: float = 0.0
    enhancement_loss_sum: float = 0.0
    total_samples: int = 0
    correct: int = 0
    ...
    def update(
        ...
        *,
        ...
        enhancement_loss: float,
        predictions: torch.Tensor,
        targets: torch.Tensor,
    ) -> None:
        """Update the running metrics from one mini-batch."""
        ...
        self.total_loss_sum += total_loss * batch_size
        self.detection_loss_sum += detection_loss * batch_size
        self.enhancement_loss_sum += enhancement_loss * batch_size
        ...
        counts = torch.bincount(matrix_indices, minlength=self.num_classes * self.num_classes)
        self.confusion_matrix += counts.view(self.num_classes, self.num_classes)

    def compute(self) -> dict[str, float | int | torch.Tensor]:
        """Return average losses and classification metrics."""
        total_loss = safe_divide(self.total_loss_sum, self.total_samples)
        detection_loss = safe_divide(self.detection_loss_sum, self.total_samples)
        enhancement_loss = safe_divide(self.enhancement_loss_sum, self.total_samples)
        accuracy = safe_divide(self.correct, self.total_samples)
        precision = safe_divide(self.true_positives, self.true_positives + self.false_positives)
        recall = safe_divide(self.true_positives, self.true_positives + self.false_negatives)
...
def format_joint_metric_line(name: str, metrics: Mapping[str, float]) -> str:
    """Format the main joint-training metrics for epoch logging."""
    return (
        f"{name:<5} "
        f"total: {metrics['total_loss']:.4f} | "
        f"detection: {metrics['detection_loss']:.4f} | "
        f"enhancement: {metrics['enhancement_loss']:.4f} | "
